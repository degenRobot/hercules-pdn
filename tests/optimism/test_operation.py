...
def offSetDebtRatioLow(strategy_mock_oracle, lp_token, token, Contract, swapPct, router, shortWhale):
    ...
    if router.address == '0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9' :
        solidRouter = interface.ISolidlyRouter01(router.address)
        solidRouter.swapExactTokensForTokensSimple(swapAmt, 0, short, token, False, shortWhale, 2**256-1, {"from": shortWhale})
    else :
...
def offSetDebtRatioHigh(strategy_mock_oracle, lp_token, token, Contract, swapPct, router, whale):
    ...
    if router.address == '0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9' :
        solidRouter = interface.ISolidlyRouter01(router.address)
        solidRouter.swapExactTokensForTokensSimple(swapAmt, 0, token, short, False, whale, 2**256-1, {"from": whale})
    else :
...
def test_reduce_debt_0(
    chain, gov, token, vault, strategy, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, router, whale
):
    # Deposit to the vault and harvest
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
    ...
    strategy.harvest()
    assert strategy.estimatedTotalAssets() < amount * 0.0001 # near zero
...
def test_lossy_withdrawal(
    chain, gov, accounts, token, vault, strategy, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, router, whale
):
    # Deposit to the vault and harvest
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
...
def test_lossy_withdrawal_95pc(
    chain, gov, accounts, token, vault, strategy, user, strategist, amount, RELATIVE_APPROX, conf, deployed_vault, lp_token, router, whale
):
...
def test_reduce_debt_with_low_calcdebtratio(
    chain, gov, token, vault, strategy_mock_oracle, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, lp_whale, lp_farm, lp_price, pid, router, shortWhale, strategy_mock_initialized_vault
):
    swapPct = 0.015
    ...
    chain.mine(1)
    strategy_mock_oracle.harvest()
    assert strategy_mock_oracle.estimatedTotalAssets() < amount * 0.0001 # near zero



def test_reduce_debt_with_high_calcdebtratio(
    chain, gov, token, vault, strategy_mock_oracle, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, lp_whale, lp_farm, lp_price, pid, router, whale, strategy_mock_initialized_vault
):
    swapPct = 0.015
    # Setting higher testPriceSource treshold for user
    strategy_mock_oracle.setSlippageConfig(9900, 400, 500, True)
...
def test_increase_debt_with_low_calcdebtratio(
    chain, gov, token, vault, strategy_mock_oracle, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, lp_whale, lp_farm, lp_price, pid, router, shortWhale
):
    # Deposit to the vault and harvest
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
...
def test_increase_debt_with_high_calcdebtratio(
    chain, gov, token, vault, strategy_mock_oracle, user, strategist, amount, RELATIVE_APPROX, conf, lp_token, lp_whale, lp_farm, lp_price, pid, router, whale
):
    # Deposit to the vault and harvest
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
