...
def test_debt_rebalance(chain, accounts, token, deployed_vault, strategy, user, conf, gov, lp_token, lp_whale, lp_farm, lp_price, pid):
    ...
    whaleBalance = lp_token.balanceOf(lp_whale)
    sendAmount = min(sendAmount, whaleBalance)
    lp_token.transfer(strategy, sendAmount, {'from': lp_whale})
    print('Send amount: {0}'.format(sendAmount))
    print('debt Ratio:  {0}'.format(strategy.calcDebtRatio()))
    ...
    auth = accounts.at(strategy, True)
    farmWithdraw(lp_farm, pid, strategy, sendAmount, conf)
...
def test_debt_rebalance_partial(chain, accounts, token, deployed_vault, strategy, user, strategist, gov, lp_token, lp_whale, lp_farm, lp_price, pid, conf):
    ...
    #assert pytest.approx(debtRatio, rel=1e-3) == (debtRatioBefore + 10000) * 0.5
