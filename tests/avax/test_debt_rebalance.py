...
def test_debt_rebalance(chain, accounts, token, deployed_vault, strategy, user, conf, gov, lp_token, lp_whale, lp_farm, lp_price, pid, collatTarget, collatLimit):
    ###################################################################
    # Test Debt Rebalance
    ###################################################################
...
def test_debt_rebalance_partial(chain, accounts, token, deployed_vault, strategy, user, strategist, gov, lp_token, lp_whale, lp_farm, lp_price, pid, collatTarget, collatLimit):
    ...
    print('collatRatio: {0}'.format(collatRatioBefore))
    assert pytest.approx(9500, rel=1e-3) == debtRatio
    assert pytest.approx(collatTarget, rel=1e-3) == collatRatioBefore
    chain.sleep(1)
    chain.mine(1)
    chain.sleep(1)
    chain.mine(1)
    ...
    assert pytest.approx(collatTarget, rel=1e-3) == collatRatio
    ...
    assert pytest.approx(collatTarget, rel=1e-3) == strategy.calcCollateral()
