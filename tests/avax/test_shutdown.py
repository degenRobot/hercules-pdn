...
def test_vault_shutdown_can_withdraw(
    ...
    assert pytest.approx(strategy.estimatedTotalAssets(), rel=5 * RELATIVE_APPROX) == amount - delta_debt
    ...
    assert pytest.approx(token.balanceOf(user), rel=5 * RELATIVE_APPROX) == amount - delta_debt
...
def test_basic_shutdown(
    ...
    chain.mine(1)
    print("Balance Lend before: ", strategy.balanceLend())


    ## Set emergency
    strategy.setEmergencyExit({"from": strategist})
    strategy.harvest()  ## Remove funds from strategy
    print("Balance Lend after: ", strategy.balanceLend())
