...
def test_partialWithdrawal_unbalancedDebtLow(
    ...
    strategy_mock_oracle.setSlippageConfig(9900, 500, 500, True)
...
def test_withdraw_all_from_multiple_strategies(
    gov, vault, strategy_mock_oracle, token, user, amount, conf, chain, strategy_contract, strategist, StrategyInsurance, keeper, collatTarget, collatLimit
):
    # Deposit to the vault and harvest
...
###TODO Incomplete test, also, we need to rewrite setOracleShortPriceToLpPrice(strategy_mock_oracle)
# def test_collat_rebalance_PriceOffset(chain, accounts, token, strategist, strategy_mock_initialized_vault, strategy_mock_oracle, user, conf, gov, lp_token, lp_whale, lp_farm, lp_price, pid, router, whale, shortWhale, collatTarget, collatLimit):
#     # set low collateral and rebalance
#     target = collatTarget - 800
#     strategy_mock_oracle.setCollateralThresholds(target-200, target, target+200, collatLimit)
