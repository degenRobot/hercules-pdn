...
def test_set_collat_thresholds(
    chain,
    accounts,
    token,
    deployed_vault,
...
def test_large_collat_rebalance_with_low_debt(
    chain,
    accounts,
    token,
    deployed_vault,
    ...
    sendAmount = round(strategy.balanceLpInShort() * (1 / 0.98 - 1))
...
def test_typical_rebalance_with_low_debt(
    chain,
    accounts,
    token,
    deployed_vault,
...
# this is a typical collat rebalance ~10% rebalance
def test_typical_collat_rebalance_with_high_debt(
    chain,
    accounts,
    token,
    deployed_vault,
