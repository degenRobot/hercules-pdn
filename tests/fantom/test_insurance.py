...
def test_report_profit(
    ...
    assert (
        pytest.approx(0.1, rel=1e-1) == token.balanceOf(strategy.insurance()) / profit
    )
...
def test_multiple_insurance_payouts(
    ...
    while True:
        ...
        max_payout = int(
            vault.strategies(strategy)[6] * insurance.maximumCompenstionRate() / 10000
        )
    ...
    assert init_insurance_balance - token.balanceOf(insurance) == pytest.approx(
        stolen, rel=1e-3
    )
...
def test_payout_then_profit(
    ...
    # send some funds to insurance for the payment
    token.transfer(
        strategy.insurance(), int(vault.totalAssets() * 0.02), {"from": whale}
    )
