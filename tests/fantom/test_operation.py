...
def offSetDebtRatioHigh(strategy, lp_token, token, Contract, swapPct, router, whale):
    ...
    router.swapExactTokensForTokens(
        swapAmt, 0, [token, short], whale, 2**256 - 1, {"from": whale}
    )
...
def steal(stealPercent, strategy, token, chain, gov, user):
    ...
    token.transfer(
        user, strategy.balanceOfWant(), {"from": accounts.at(strategy, True)}
    )
    chain.sleep(1)
    chain.mine(1)
...
def test_change_debt_lossy(
    ...
    strategy.harvest()
    assert pytest.approx(strategy.estimatedTotalAssets(), rel=1e-2) == int(
        amount * 0.98 / 2
    )
...
def test_lossy_withdrawal(
    ...
    assert pytest.approx(balAfter - balBefore, rel=2e-3) == int(amount * 0.99)
