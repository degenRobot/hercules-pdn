...
def offSetDebtRatioLow(strategy, lp_token, token, Contract, swapPct, router, shortWhale):
    ...
    router.swapExactTokensForTokens(
        swapAmt, 0, [short, token], shortWhale, 2**256 - 1, {"from": shortWhale}
    )
...
def test_triggers(chain, gov, vault, strategy, token, amount, user, conf):
    # Deposit to the vault and harvest
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
...
def test_reduce_debt_with_low_calcdebtratio(
    ...
    strategy_mock_oracle.harvest()
    assert strategy_mock_oracle.estimatedTotalAssets() < 10 ** (
        token.decimals() - 3
    )  # near zero
...
def test_reduce_debt_with_high_calcdebtratio(
    ...
    strategy_mock_oracle.setSlippageConfig(9900, 400, 500, True)
    offSetDebtRatioHigh(
        strategy_mock_oracle, lp_token, token, Contract, swapPct, router, whale
    )
    # setOracleShortPriceToLpPrice(strategy_mock_oracle)
    debtRatio = strategy_mock_oracle.calcDebtRatio()
    collatRatioBefore = strategy_mock_oracle.calcCollateral()
    ...
    assert pytest.approx(strategy_mock_oracle.estimatedTotalAssets(), rel=2e-3) == int(
        newAmount / 2
    )
...
def test_increase_debt_with_low_calcdebtratio(
    ...
    strategy_mock_oracle.harvest()
    assert (
        pytest.approx(strategy_mock_oracle.estimatedTotalAssets(), rel=RELATIVE_APPROX)
        == half
    )
    ...
    strategy_mock_oracle.harvest()
    assert (
        pytest.approx(strategy_mock_oracle.estimatedTotalAssets(), rel=2e-3) == amount
    )
