...
WETHWHALE =  "0x50eaEDB835021E4A108B7290636d62E9765cc6d7"
...
def strategy_contract():
    yield project.CoreStrategyProject.USDCWETHQuick
...
def whale(token, conf, Contract, accounts):
    if conf["router"] == QUICKSWAP_ROUTER:
    ...
    whale = factory.getPair(token, secondToken)

    yield accounts.at(whale, True)
...
@pytest.fixture
def lp_farm(conf):
    yield interface.IStakingDualRewards(conf["lp_farm"])
...
@pytest.fixture
def strategy_mock_initialized_vault(
    chain,
    accounts,
    gov,
    token,
...
def strategy_mock_oracle(
    ...
    vault.addStrategy(
        strategy_mock_oracle, 10_000, 0, 2**256 - 1, 1_000, {"from": gov}
    )
