...
def run_debt_rebalance_price_offset(
    price_offset,
    accounts,
    ...
    sendAmount = round(strategy_mock_oracle.balanceLp() * (1 / 0.95 - 1) / lp_price)
    ...
    sendAmount = round(strategy_mock_oracle.balanceLp() * (1 / 0.4 - 1) / lp_price)
    lp_token.transfer(strategy_mock_oracle, sendAmount, {"from": lp_whale})
    print("Send amount: {0}".format(sendAmount))
    print("debt Ratio:  {0}".format(strategy_mock_oracle.calcDebtRatio()))
...
def run_debt_rebalance_partial_price_offset(
    price_offset,
    accounts,
    strategy,
    user,
    ...
    # Change the debt ratio to ~95% and rebalance
    sendAmount = round(strategy.balanceLpInShort() * (1 / 0.95 - 1))
...
def test_debt_rebalance_price_offset_high(
    accounts,
    strategy_mock_oracle,
    ...
    run_debt_rebalance_price_offset(
        1.1,
        accounts,
        strategy_mock_oracle,
        user,
...
def test_debt_rebalance_price_offset_low(
    accounts,
    strategy_mock_oracle,
    strategy_mock_initialized_vault,
    user,
...
def test_debt_rebalance_partial_price_offset_high(
    accounts,
    strategy_mock_oracle,
    strategy_mock_initialized_vault,
    user,
    ...
    run_debt_rebalance_partial_price_offset(
        1.1,
        accounts,
        strategy_mock_oracle,
        user,
...
def test_debt_rebalance_partial_price_offset_low(
    accounts,
    strategy_mock_oracle,
    strategy_mock_initialized_vault,
    user,
