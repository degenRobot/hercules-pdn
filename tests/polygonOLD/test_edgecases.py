...
def steal(stealPercent, strategy_mock_oracle, token, chain, gov, user):
    ...
    token.transfer(
        user,
        strategy_mock_oracle.balanceOfWant(),
        {"from": accounts.at(strategy_mock_oracle, True)},
    )
    chain.sleep(1)
    chain.mine(1)


def offSetDebtRatioLow(
...
def strategySharePrice(strategy_mock_oracle, vault):
    return (
        strategy_mock_oracle.estimatedTotalAssets()
        / vault.strategies(strategy_mock_oracle)["totalDebt"]
    )
...
def test_collat_rebalance_PriceOffset(
    chain,
    accounts,
    token,
    strategist,
    ...
    target = 4500
    strategy_mock_oracle.setCollateralThresholds(
        target - 200, target, target + 200, 7500
    )
