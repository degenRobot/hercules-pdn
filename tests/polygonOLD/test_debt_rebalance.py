...
def test_debt_rebalance(
    ...
    sendAmount = round(strategy.balanceLp() * (1 / 0.95 - 1) / lp_price)
    ...
    sendAmount = round(strategy.balanceLp() * (1 / 0.4 - 1) / lp_price)
    lp_token.transfer(strategy, sendAmount, {"from": lp_whale})
    print("Send amount: {0}".format(sendAmount))
    print("debt Ratio:  {0}".format(strategy.calcDebtRatio()))
...
def test_debt_rebalance_partial(
    chain,
    accounts,
    token,
    deployed_vault,
