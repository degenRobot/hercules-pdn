...
def test_vault_shutdown_can_withdraw(
    ...
    assert (
        pytest.approx(token.balanceOf(user), rel=RELATIVE_APPROX) == amount - delta_debt
    )
...
def test_basic_shutdown(
    ...
    dust = amount * 0.00001
    assert (
        strategy.estimatedTotalAssets() < dust
    )  ## the strat shouldn't have more than some dust
