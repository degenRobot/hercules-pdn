...
abstract contract CoreStrategyAaveSolidly is BaseStrategy {
    ...
    uint8 wantDecimals;
    uint8 shortDecimals;
    IBaseV1Pair wantShortLP; // This is public because it helps with unit testing
    IERC20 farmTokenLP;
    IERC20 farmToken;
    // Contract Interfaces
    address farmMasterChef; //Since it is usually custom, will leave it as an address
    ISolidlyRouter01 router;
        ...
        BaseStrategy(_vault)
    {
        ...
        wantShortLP = IBaseV1Pair(_config.wantShortLP);
        ...
        router = ISolidlyRouter01(_config.router);
        weth = router.weth();
    ...
    function getTokenOutPath(address _token_in, address _token_out)
        internal
        view
        returns (Route[] memory _routes)
    {
        bool is_weth =
            _token_in == address(weth) || _token_out == address(weth);
        address[] memory _path = new address[](is_weth ? 2 : 3);
        _path[0] = _token_in;
        if (is_weth) {
            _path[1] = _token_out;
        } else {
        ...
        _routes = convertPathRoutes(_path);
    }

    function convertPathRoutes(address[] memory _path)
        internal
        view
        returns (Route[] memory _routes)
    {
        bool isLong = _path.length == 3;
        _routes = new Route[](isLong ? 2 : 1);

        _routes[0] = Route(_path[0], _path[1], false);

        if (isLong) {
            _routes[1] = Route(_path[1], _path[2], false);
        }
    }
        ...
        returns (uint256 _slippageWant)
    {
        ...
        /// solid router doesn't have swapTokensForExactTokens
        (uint256 _expectedOut, ) =
            router.getAmountOut(amountInWant, address(want), address(short));
        uint256 amountInMax =
            amountInWant
                .mul(_amountOut)
                .div(_expectedOut)
